...
#bip pwd zKSqSHd5
class RemotePayonTest < Test::Unit::TestCase
  ...
  def test_successful_void
    ...
    assert_success auth
    @gateway.logger.info("#{__method__.to_s} - authorization response #{auth.params}")
    assert_equal @@success_message[@mode], auth.message

    assert void = @gateway.void(auth.authorization)
    assert_success void
    @gateway.logger.info("#{__method__.to_s} - void response #{void.params}")
  ...
  def test_successful_purchase
    ...
    @gateway.logger.info("#{__method__.to_s} - response #{response.params}")
  ...
  def test_successful_refund
    ...
    @gateway.logger.info("#{__method__.to_s} - purchase response #{purchase.params}")

    assert refund = @gateway.refund(nil, purchase.authorization)
    assert_success refund
    @gateway.logger.info("#{__method__.to_s} - refund response #{refund.params}")
  ...
  def test_successful_authorize_and_capture
    ...
    @gateway.logger.info("#{__method__.to_s} - capture response #{capture.params}")
